<template>
  <div class="weatherInfo">
    ...
    <button :class="['weatherInfo__button', {favorite: isFavorite}]" @click.prevent="toggleFavorite">
...
export default {
  ...
  computed: {
    ...
    icon() {
      const iconName = this.weatherInfo.weather[0].icon.match(/\d*/)[0];
  ...
  methods: {
    ...
    toggleFavorite() {
      if (this.isFavorite) {
        this.deleteFavoriteCity(this.cityName);
      } else {
        this.addFavoriteCity(this.cityName);
      }
      this.changeFavoriteState(!this.isFavorite);
    },
  },
}
</script>
